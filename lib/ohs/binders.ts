 ...
 * (FM-TC-01, Sept 2024) used with the WorkSafeBC NOP-TC. 1415 does not
 * replace those templates. The wizard builds the 1415 copy: numbered
...
const SHARED = {
  ...
  tharrp: (n: string): BinderItem => ({
    ...
    group: PRE,
    need: "required",
    note: "A tower operator in the cab is fire-department rope rescue, not a 1415 rappel. Host or prime requests the survey in the THARRP portal. Confirm it exists before the operator goes up.",
  ...
  swpAssembly: (n: string): BinderItem => ({
    ...
    group: PROC,
    need: "required",
    note: "Written procedures for the high-risk work. 1415-SWP-027 is the company method. The site procedure names this pad, this assist crane, this crew.",
    whoop: [whoop("/safety/swp/tower-erection-climbing", "SWP-027 →")],
  }),
  ...
  pm: (n: string): BinderItem => ({
    ...
    group: POST,
    need: "required",
    note: "OEM interval for this serial. 1415 weekly and monthly forms record the work. The schedule in the binder is the manufacturer’s, not a guess.",
    whoop: [
...
export function binderItemLinks(item: BinderItem): DocLink[] {
  const fill = item.formSlug
    ? [
        whoop(`/safety/form/${item.formSlug}`, "1415 FORM →"),
