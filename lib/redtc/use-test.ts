...
import type { Question, TestState, TestResult } from "./types";

const DEFAULT_QUESTIONS_PER_TEST = 10;
const DEFAULT_PASS_PERCENTAGE = 70;
const DEFAULT_SEEN_KEY = "redtc-seen-questions";
...
interface TestOptions {
  ...
  examMode?: boolean;
  /** Keep Tower and Mobile seen-question sets apart. */
  seenKey?: string;
}
...
function getSeenQuestions(key: string): Set<number> {
...
function saveSeenQuestions(key: string, ids: number[]): void {
...
function selectWeightedRandomQuestions(
  questions: Question[],
  count: number,
  seenKey: string,
): Question[] {
  const seenIds = getSeenQuestions(seenKey);
  ...
  if (uniqueSeenIds.length >= questions.length * 0.9) {
    saveSeenQuestions(seenKey, selected.map(q => q.id));
  } else {
    saveSeenQuestions(seenKey, uniqueSeenIds);
  }
...
export function useTest(allQuestions: Question[], options: TestOptions = {}) {
  ...
  const seenKey = options.seenKey || DEFAULT_SEEN_KEY;
  
  const [testQuestions, setTestQuestions] = useState<Question[]>(() => 
    selectWeightedRandomQuestions(allQuestions, questionsPerTest, seenKey)
  );
  ...
  const resetTest = useCallback(() => {
    setTestQuestions(selectWeightedRandomQuestions(allQuestions, questionsPerTest, seenKey));
    setState({
      currentQuestionIndex: 0,
      answers: {},
      showExplanation: false,
    ...
    questionStartTime.current = Date.now();
  }, [allQuestions, questionsPerTest, seenKey]);
